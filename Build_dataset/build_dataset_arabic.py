...
def getEntitiesArabic(sources):
    ...
    for sentence in sources:
        token = simple_word_tokenize(sentence)
        if(len(token)<100):
            tokenized_sentences.append(simple_word_tokenize(sentence))
        # if(len(tokens) > 150):
        #     continue
        # try:
    ...
    labels = ner.predict(tokenized_sentences)
    print('labelled all sentences!')
...
#         s = file.readline()
# else:
sources_full = file.read().split("\n")
chunk_size=int(args.num_sentences)
for i in range(len(sources_full)//chunk_size):
    try:
        sources = sources_full[i*chunk_size:(i+1)*chunk_size]
        sources_tagged = getEntitiesArabic(sources)
        ...
        print(final_texts[0])
        file_final = open("final_texts_arabic_wiki.txt","a",encoding = "utf-8")
